...
ft_write:
    ...
    jl invalid_fd           ; Jump to invalid_fd section
    ...
    jl write failed         ; Jump to write_failed if failed
...
invalid_fd:
    ...
    mov dword [eax], 9      ; Set errno to EBADF (Bad file descriptor, value 9)
    jmp error               ; Jump to error handling

invalid_size:
    call __errno_location   ; Get the address of errno
    mov dword [eax], 22     ; Set errno to EINVAL (Invalid argument, value 22)
    jmp error               ; Jump to error handling

write_failed:
    call __errno_location   ; Get the address of errno
    mov dword [eax], 5      ; Set errno to EIO (I/O error, value 5)
    jmp error               ; Jump to error handling
